...
describe('AggregationClient', () => {
  ...
  describe('execute', () => {
    ...
    test('should handle API errors', async () => {
      mockAxiosInstance.mockRejectedValueOnce(
        new Error('Aggregation request failed: 400 Bad Request - Invalid filter expression')
      );
  ...
  describe('complex queries', () => {
    test('should build complex query correctly', async () => {
      ...
      expect(mockAxiosInstance).toHaveBeenCalledTimes(1);
      const requestBody = mockAxiosInstance.mock.calls[0][0].data;
